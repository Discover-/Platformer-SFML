...
namespace Collision
{
		...
		if (Object1.getGlobalBounds().intersects(Object2.getGlobalBounds(), Intersection))
        {
			...
			for (int i = int(Intersection.left); i < int(Intersection.left+Intersection.width); i++)
            {
				for (int j = int(Intersection.top); j < int(Intersection.top+Intersection.height); j++)
                {
					...
					if (o1v.x > 0 && o1v.y > 0 && o2v.x > 0 && o2v.y > 0 && o1v.x < O1SubRect.width && o1v.y < O1SubRect.height && o2v.x < O2SubRect.width && o2v.y < O2SubRect.height)
						if (Bitmasks.GetPixel(mask1, Object1.getTexture(), (int)(o1v.x)+O1SubRect.left, (int)(o1v.y)+O1SubRect.top) > AlphaLimit && Bitmasks.GetPixel(mask2, Object2.getTexture(), (int)(o2v.x)+O2SubRect.left, (int)(o2v.y)+O2SubRect.top) > AlphaLimit)
							return true;
				}
			}
		}
	...
	bool BoundingBoxTest(const sf::Sprite& Object1, const sf::Sprite& Object2)
    {
		...
		float CosA = float(cos(Angle * RadToDeg));
		float SinA = float(sin(Angle * RadToDeg));
		...
		float O2Rot = Object2.getRotation() * float(RadToDeg);
		float temp = C.x;
